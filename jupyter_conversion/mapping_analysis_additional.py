...
#   kernelspec:
#     display_name: Python [conda env:thesis_analysis] *
#     language: python
#     name: conda-env-thesis_analysis-py
# ---
...
np.hstack(df['keywords'].str.split(',')).tolist()

np.unique(np.hstack(df['keywords'].str.split(','))).tolist()
...
corpus = np.array([np.unique(np.hstack(df['keywords'].str.split(','))).tolist()])
corpus.T
new = np.append(corpus.T, corpus.T, axis=1)
new
